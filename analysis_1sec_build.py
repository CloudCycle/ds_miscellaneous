...
def create_event(data, run_type='not-live'):
    ...
    input_json['system_datetime_utc_str_start'] = [data['system_datetime_utc_str'].min()]
...
def add_geofence_base(df_batch, time_cutoff_min=15):
    ...
    time_since_start = (df_batch['datetime_utc'] - df_batch['datetime_utc'].min()).dt.seconds
    speed_drop_time = df_batch['datetime_utc'].loc[(df_batch['drum_speed_mean_rpm'] < 5) & (df_batch['drum_speed_mean_rpm'] >= 1) & (time_since_start <= time_cutoff_min*60)] 
    
    for t_start in speed_drop_time:
        t_end = t_start + pd.Timedelta(1, 'm')
        df_1_min = df_batch.query("(datetime_utc >= @t_start) & (datetime_utc <= @t_end)")
        if df_1_min['drum_speed_mean_rpm'].std() <= 0.15:
            df_batch.loc[df_batch['datetime_utc'] <= t_start, 'geofence_type_lastvalid'] = 'BASE'
            return df_batch
        else:
            continue
...
def batch_predict_pipeline_trips(df_batch:pd.DataFrame) -> pd.DataFrame:
    ...
    data = (df_batch
           ...
           .assign(run_id="not-live",
                   system_datetime_posix_utc_seconds = lambda x: x['system_datetime_utc_str'].values.astype(np.int64) // 10 ** 9,
                   truck_project = 'Cemex'
                   )
...
def plot_fundamentals(df):
    ...
    f_rug = (hv.Scatter(df_plot.assign(rug=0), 'system_datetime_utc_str', 'rug', label='').opts(color='black', marker='dash', angle=90, size=5))
...
def extend_trip_start(df_trip, df_reference, time_sec=600):
    ...
    df_trip_extended = df_reference.xs((trip_id[0], slice(required_start, trip_end)), level=[0,2], drop_level=False)
...
if __name__ == '__main__':
    ...
    truck = 'KX67RKJ'
    df_ = (redshift_client
          .execute_redshift_query(query_augmented_log
    ...
    res = dict()
    for (truck, trip_start), df_trip in df.groupby(level=[0,1]):
        
        # if trip_start not in plot_only: continue            
        df_trip = extend_trip_start(df_trip, df, 60)
        overlay = plot_fundamentals(df_trip)*hv.VLine(pd.to_datetime(trip_start,unit='s',utc=True))
        
        slump_code = [i for i in df_trip['docket_slump_code'].unique() if '<empty>' not in i]
        if len(slump_code):
            sc = slump_code[0]
        else:
            sc = '<empty>'
        title = f"{truck}, {str(from_timestamp(trip_start))}, {sc}"
        opts = [hv.opts.Overlay(width=500, height=350, ylim=(-50,250), show_grid=True, title=title), 
        ...
        df_pred = batch_predict_pipeline_trips(df_trip.reset_index().assign(geofence_type_lastvalid = '<empty>'))
        df_pred = (df_pred
                   .assign(system_datetime_utc_str=lambda x: pd.to_datetime(x['system_datetime_utc_str'],utc=True))
                  .set_index(['truck_registration', 'prediction_trip_posix_timestamp_start', 'system_datetime_utc_str'])
                  )
        df_merged = (df_trip
                    .join(df_pred, how='left', rsuffix='_pred')
                    .assign(prediction_slump_mm_current = lambda x: np.maximum(-10, x['prediction_slump_mm']))
                    .sort_index()
                    )
    ...
    # ------------------------------------------------------------------ #
    start_date = np.datetime64("2022-10-10")
    end_date = np.datetime64("2022-10-11")
    ...
    time_slice = slice(pd.to_datetime('2022-10-10T15:39'), pd.to_datetime('2022-10-10T16:00'))
    df = (df_
          .assign(system_datetime_utc_str=lambda x: pd.to_datetime(x['system_datetime_utc_str'], utc=True))
          .set_index(['truck_registration', 'prediction_trip_posix_timestamp_start', 'system_datetime_utc_str'])
          .xs(time_slice, level=2, drop_level=False)
         )
    
    
    opts = [hv.opts.Overlay(width=500, height=350, ylim=(-50,250), show_grid=True, title=df['imei'].unique()[0]), hv.opts.Scatter(tools=['hover'])]
    for (truck, trip_start), df_trip in df.groupby(level=[0,1]):
        overlay = plot_fundamentals(df_trip)
        
        f_press = (hv.Scatter(df_trip, 'system_datetime_utc_str', 'pressure_a_bar_mean', label='pressure_a_bar_mean').opts(color=colors[4], show_grid=True)
                *hv.Curve(df_trip, 'system_datetime_utc_str', 'pressure_a_bar_mean', label='pressure_a_bar_mean').opts(color=colors[4], alpha=0.5))

        overlay = overlay.Scatter.Rpmx10*overlay.Curve.Rpmx10
        display(overlay.opts(opts))
        display(truck, trip_start, from_timestamp(trip_start))
        
    # ------------------------------------------------------------------ #
    start_date = np.datetime64("2022-10-14")
    end_date = np.datetime64("2022-10-15")
